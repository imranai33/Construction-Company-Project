...
const Accountedit = ({ editId, onSuccess }) => {
  const {
    register,
    handleSubmit,
    reset,
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (result.status) {
        ...
        reset();
        if (onSuccess) onSuccess(); // e.g., redirect or refresh table
    ...
    <div className="max-w-3xl mx-auto bg-white shadow-lg rounded-xl p-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div>
          ...
          />
          {errors.father && (
            <p className="text-red-500 text-sm">{errors.father.message}</p>
          )}
        </div>
